...
$terrain_images = array("tile1.png", "tile2.png");
...
<style>
	...
	.card , .active_card {
		filter: drop-shadow(0px 0px 15px rgb(0 0 0 / 0.66));
	}
	.card_popup {
		float: left;
...
<script>
	...
	function select_card(card_file, hand_id, card_name, card_desc) {
		...
		document.getElementById('active_card').src = card_file;
		document.getElementById('card_popup_name').textContent = card_name;
		document.getElementById('card_popup_desc').textContent = card_desc;
	}
...
if(!empty($_POST['draw_card']) AND sizeof($_SESSION['hand']) < $card_limit) {
	...
	# this is disabled so i can work on unifying code for cards
	if(mt_rand(1,5) == 0) {
		$drawn_card = str_pad(mt_rand(0, 18), 3, 0, STR_PAD_LEFT) . ".png";
	} else {
		$drawn_card = $playing_cards[array_rand($playing_cards)];
	}
	# no repeats, hacky solution that doesnt account for duplicates in deck
	# decks should isntead be copied to arrays that are unset as cards are pulled, maybe
...
echo "<div class='main_area'>
	<div class='discard_pile'>
		...
		<img src='./cards/";
			if(sizeof($_SESSION['discard']) == 0) {
				...
				echo $_SESSION['discard'][sizeof($_SESSION['discard']) - 1]['file'];
			}
			echo "' width='200px' />
	</div>
	<div class='inline'>
	...
	<div class='inline' style='width: auto; height: 325px; margin-left: -20px; transform: translate(0, -260px);'>
		...
		foreach($_SESSION['hand'] as $card) {
			echo "<li>
				<img src='./cards/{$card['file']}' width='200px' class='card' style=' 
					translate: " . $i * 30 . "% 50%;
					position: absolute;
					top: 50%;
				...
				'
				onclick='select_card(&#39;./cards/" . $card['file'] . "&#39;, " . array_search($card, $_SESSION['hand']) . ", &#39;" . $card['name'] . "&#39;, &#39;" . $card['desc'] . "&#39;)'/>
			</li>";
			$i += 1;
		}
		echo "</ul>
